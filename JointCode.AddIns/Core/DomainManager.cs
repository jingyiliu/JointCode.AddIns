...
namespace JointCode.AddIns.Core
{
    class DomainManager : IDisposable
    {
        ...
        {
            var obj = CreateMarshalObject(domain, typeof(T).Assembly.Location, typeof(T).FullName);
            return obj as T;
        }

        //assemblyParam: might be assembly path or assembly name where the marshalTypeFullName resides
        public object CreateMarshalObject(AppDomain domain, string assemblyParam, string marshalTypeFullName)
        {
            if (assemblyParam.EndsWith(".dll", StringComparison.CurrentCultureIgnoreCase) ||
                assemblyParam.EndsWith(".exe", StringComparison.CurrentCultureIgnoreCase))
            {
                try
                {
                    AssemblyName assemblyName;
                    if (File.Exists(assemblyParam))
                        assemblyName = AssemblyName.GetAssemblyName(assemblyParam);
                    else
                        return null;
                    return domain.CreateInstanceAndUnwrap(assemblyName.FullName, marshalTypeFullName);
                }
                catch
                {
                    return null;
            ...
            else
            {
                try
                {
                    return domain.CreateInstanceAndUnwrap(assemblyParam, marshalTypeFullName);
                }
                catch
                {
                    return null;
